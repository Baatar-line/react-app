// Thin client for the Express/Prisma backend in ../../server.
// AdminPanel has no real login screen yet, so it authenticates itself with a
// seeded dev admin account (see server/prisma/seed.ts) and caches the token —
// same idea as any other session token, just bootstrapped instead of typed in.
...
async function login(): Promise<string> {
  ...
  if (!res.ok) throw new Error('Admin login failed — is the backend running and seeded? (npm run seed in /server)');
...
// Must stay <= the multer fileSize limit in server/src/middleware/upload.ts.
// Checking it client-side lets us fail with a clean message instead of a raw
// "Failed to fetch" — a request that blows past the server's limit gets its
// connection cut mid-upload, which the browser reports as a network error
// rather than delivering the 413 the server tried to send.
